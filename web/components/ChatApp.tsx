...
export function ChatApp({ initialTheme }: { initialTheme?: string }) {
  ...
  const initialThemeSentRef = useRef(false);
  ...
  useEffect(() => {
    if (initialTheme && !initialThemeSentRef.current) {
      initialThemeSentRef.current = true;
      send(initialTheme);
    }
